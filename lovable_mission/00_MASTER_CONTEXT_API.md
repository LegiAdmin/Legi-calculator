...
```typescript
// --- ENUMS ---
type OwnershipMode = "FULL_OWNERSHIP" | "USUFRUCT" | "BARE_OWNERSHIP" | "INDIVISION";
type AssetOrigin = "PERSONAL_PROPERTY" | "COMMUNITY_PROPERTY" | "INHERITANCE" | "INDIVISION";
type UsufructType = "TEMPORARY" | "LIFETIME"; // Exemple, à définir précisément si nécessaire
type SubscriberType = "DECEASED" | "SPOUSE" | "OTHER"; // Exemple, à définir précisément si nécessaire
type ProfessionalExemption = "ART_787_B" | "ART_787_C" | "OTHER"; // Exemple, à définir précisément si nécessaire

type LifeInsuranceContractType = 
  | "STANDARD" // Défaut
  | "VIE_GENERATION" // Abattement 20% supp. (Art. 990 I bis CGI)
  | "ANCIEN_CONTRAT"; // Exonéré (Primes < 1998 / Contrat < 1991)
...
interface Asset {
  ...
  // Démembrement
  usufructuary_birth_date?: string;
  ...
  spouse_occupies_property?: boolean;
  
  // Assurance-vie
  premiums_before_70?: number;
  premiums_after_70?: number;
  subscriber_type?: SubscriberType;
  life_insurance_contract_type?: LifeInsuranceContractType;
  ...
  location_country?: string; // ISO 2 (ex: 'US', 'BE')

  // Droit de Retour (Art. 738-2 CC)
  received_from_parent_id?: string; // ID du parent donateur (si bien reçu par donation)
}

interface FamilyMember {
  id: string;                          // Identifiant unique
  birth_date: string;                  // "YYYY-MM-DD"
  relationship: "CHILD" | "SPOUSE" | "PARTNER" | "PARENT" | "SIBLING" | "GRANDCHILD" | "GREAT_GRANDCHILD" | "NEPHEW_NIECE" | "OTHER";
  
  // Famille Recomposée
  is_from_current_union?: boolean;     // Défaut: true
  
  // Handicap (Art. 779 II CGI)
  is_disabled?: boolean;               // Abattement suppl. 159 325€

  // Représentation
  represented_heir_id?: string;        // ID du parent prédécédé représenté
  
  // Fente Successorale (Si pas de conjoint/enfants)
  paternal_line?: boolean;             // true = Ligne Paternelle, false = Maternelle
  
  // Adoption (Art. 786 CGI)
  adoption_type?: "FULL" | "SIMPLE";
  has_received_continuous_care?: boolean; // Pour adoption simple (5 ans soins continus)
  
  // Option Successorale (Refus)
